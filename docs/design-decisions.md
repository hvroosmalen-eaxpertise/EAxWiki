...
- **`--brand <name>` makes branding configurable, with EurSuRA as the first brand.** The brand is resolved in `Program.cs` from, in priority order: the `--brand` CLI flag → env `EAXWIKI_BRAND` → the `.eaxwiki` `Brand` field (PascalCase key, matching the `LocalConfigStore.Config` model; PowerShell property access in the monitor/export scripts is case-insensitive). The `export.ps1` script passes `--brand` through to the exporter verbatim; the monitor resolves it via `--brand` CLI flag → env `EAXWIKI_BRAND` → `.eaxwiki` when no flag is given.

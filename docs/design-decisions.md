...
- **Export pipeline**: C# .NET 10 console app reads EA model via COM Interop, writes Markdown + PNG files, then MkDocs serves the result. Wrapper scripts for flexibility: `export.ps1` (export only), `serve.ps1` (serve only), `export-and-serve.ps1` (both), `serve-api.ps1` (serve + write-back server), and `writeback.ps1` (batch write-back). Unattended monitoring/scheduling is a separate C# process — see [Unattended monitoring & scheduling](#unattended-monitoring--scheduling-issues-37-38).
    ...
    sequenceDiagram
        ...
        Script->>Exporter: dotnet exec EAxWiki.dll --output wiki [--api-port]
    ...
    ```
- **Test projects** (`EAxWiki.Tests`, `EAxWiki.Monitor.Tests`): 443 tests — exporter/writer unit + integration tests, `EaReader`/`ModelMapper` COM mapping, the System.CommandLine parser, write-back server HTTP tests (TestHost + `FakeEaReader`, no ports/COM), and FsCheck property-based invariants. xUnit with `InternalsVisibleTo`; the full breakdown is in the README Tests table.
...
- **Seven views**: Structure (tree hierarchy), Types (grouped by stereotype), Diagrams (global alphabetical index), Glossary (terms from tagged values and element notes), Recent Changes (top 50 most recently modified elements and diagrams), Status Dashboard (`/status/`), and Model Health (`/status/model-health.html`).
...
- **Argument parsing is System.CommandLine 2.0.11** (`src/EAxWiki/CommandLine.cs`): `Config` is a pure DTO; unknown or typo'd flags are hard parse errors (exit 1); repeated single-valued options are rejected; `--help` prints auto-generated usage. Exe option surface: `--verbose`/`-v`, `--force`/`-f`, `--json`/`-j`, `--repo`/`-r`, `--name`/`-n`, `--output`/`-o`, `--package`/`-p`, `--writeback`/`-w`, `--api`, `--api-port`, `--wiki-port`, `--cert`, `--cert-password`, `--brand`, and the `--ai-*` options.
...
- **Scripts run the pre-built DLL, never `dotnet run`**: `dotnet run --project` rebuilds and would overwrite the DLL a running write-back server has locked, so every wrapper invokes the already-built `EAxWiki.dll` via `dotnet exec` (path resolved by `Get-EAxWikiDllPath` in `scripts/_bootstrap.ps1`, with a clear "run `dotnet build` first" error when it's missing). `export.ps1`/`writeback.ps1` forward the user's `$args` verbatim; `export-and-serve.ps1`/`serve-api.ps1` parse only `--port`/`--api-port`/`--repo`/`--output` (plus the legacy `-Port`/`-ApiPort`/`-RepoPath`/`-OutputDir` aliases, normalized to canonical flags) and forward everything else untouched — a typo'd flag fails fast in the exe parser (exit 1) instead of being swallowed. Scripts run from the repo root, so a relative `--output`/`--repo` resolves predictably against it; the orchestration scripts additionally resolve `--output` to an absolute path for the write-back server job.
...
- **Monitoring runs as a separate unattended process (`EAxWiki.Monitor.exe`), not inside the exporter or the wrapper scripts**: a broken exporter can't reliably report its own breakage, so a broken/missing export must be detected by something outside it. The monitor runs the pre-built `EAxWiki.dll` in-process (no `dotnet run`/rebuild, so it never trips the DLL lock), with bounded retry (`--max-retries`, default 3) with backoff, a sanity check (`--min-element-fraction`, default 0.5) that catches an export exiting 0 while producing near-empty output, a serve watchdog, a health page, and alert dispatch. It is registered as a Windows Task Scheduler task by `register-scheduled-task.ps1`.
- **Alert dispatch is channel-agnostic and per-channel independent**: Slack, Teams, and Telegram are implemented (`--webhook-url`, `--teams-webhook-url`, `--telegram-bot-token` + `--telegram-chat-id`), each posting independently — one webhook failing (revoked, wrong URL) doesn't suppress the others, and none configured just means alerts are logged locally. A transient failure that resolves on retry within the same pass does not alert — only the final outcome of a pass does. Webhook URL resolves in order: CLI arg → env var → `.eaxwiki`; deliberately not accepted as a Task Scheduler action argument, since those are readable by any admin via `Get-ScheduledTask`.
- **`wiki/status/health.md`** (pipeline health) is a separate page from the element Status Dashboard (`wiki/status/index.md`) — different subject (export/serve pipeline health vs. EA model element status), generated by the monitor, not the exporter, since its entire job is to report when export *didn't* run.
- **Serve watchdog checks the port before the PID**: the monitor first checks whether its own tracked serve process (stored with a start-time alongside the PID, not just the bare PID, to survive a reboot without a false positive from PID reuse) is still running; if not, it probes whether the wiki port itself is already listening before concluding serve is down. Without the port check, restarting serve after the monitor's own state was lost would start a second, colliding `mkdocs serve` on top of one started manually (e.g. via `export-and-serve.ps1`) outside its tracking.
- **Day/night scheduling is two native Task Scheduler triggers on one task, not a config file** (`register-scheduled-task.ps1 --work-start/--work-end/--work-interval-minutes/--off-hours-interval-minutes`): a slow all-day/every-day baseline (so a real failure at night/on a weekend isn't silently indistinguishable from "just paused") plus a fast weekday work-hours boost layered on top. `MultipleInstances IgnoreNew` (already needed for basic overlap protection) makes the two triggers' deliberate overlap during weekday daytime harmless. The monitor needs zero changes — it has no idea day/night scheduling exists. Explicitly not timezone-aware: EA COM only runs on one Windows machine in one timezone, so "day vs night" can only mean that machine's own clock, not per-reader adaptation for a distributed team (see `docs/superpowers/specs/2026-07-03-issue-38-scheduling-design.md`).
...
- **Teams webhook support (issue #39) is additive to Slack, not a replacement or a choice between the two**: `TeamsWebhookUrl` sits alongside `WebhookUrl` in `LocalConfigStore.Config` and resolves through the identical three-tier order (CLI arg → env var → `.eaxwiki`). The alert dispatcher posts to each configured channel independently — one webhook failing (revoked, wrong URL) doesn't suppress the other, and none configured just means alerts are logged locally only. Teams uses the classic Incoming Webhook `MessageCard` payload (`@type: MessageCard`), not Slack's `attachments` structure; the two payloads are built and posted separately, sharing only the `Kind`-derived color/message, not a common serialization path — the schemas don't overlap enough to make a shared builder worth it.

## Model Health (issue #68)

- **Model content quality, not export mechanics or pipeline health**: three different "health" concepts live in `status/`, deliberately kept separate — `status/health.md` (pipeline health: did export/serve actually run, generated by the monitor), the `ExportValidator` report (did the export render correctly: broken links, unclosed HTML, missing images — issue #49), and `status/model-health.md` (does the *model itself* have content problems: orphan elements, missing descriptions, stale elements, duplicate names — generated by `ModelHealthExporter` in the C# exporter, alongside the Status Dashboard).
...
## Branding (issues #79, #97, #98)

_2026-08-04, revised 2026-09-04 (#97) and 2026-09-07 (#98)_

- **Branding is user-owned, not a code concept.** The original `--brand eursura` flag from #79 was removed in #97. There is no hard-coded brand list, no `--brand` CLI/env/config setting, and no per-brand embedded assets. Each wiki owns its look through a single file — `wiki/brand.css` — plus the `theme.logo` line in `mkdocs.yml`.
- **`wiki/brand.css` is seeded once and never overwritten** (`InfrastructureWriter.SeedBrandCssIfMissingAsync`). On first export the exporter drops a template of commented-out Material CSS variables; subsequent exports leave the file untouched. Users can edit it freely (or through the brand editor, below) and their changes survive every export cycle.
- **Brand editor on the Configuration page** (issue #98): the read-only `wiki/status/config.md` grew a widget shell (`ConfigPageRenderer.AppendBrandEditor`) that `brand-editor.js` mounts into. Six color pickers (Material's `--md-primary-fg-color`, `--md-primary-fg-color--light`, `--md-accent-fg-color`, `--md-typeset-a-color`, plus custom `--ea-section-bg` / `--ea-section-fg` for the collapsible section headers), a font input (auto-adds a Google Fonts `@import` for bare names), logo upload / remove, and a raw `brand.css` textarea as the escape hatch. The pickers manipulate a `:root { ... }` block inside the textarea (`upsertVar` / `removeVar`); Save posts the full body to `POST /api/brand-css`.
- **Logo upload lands under `wiki/assets/` and patches `mkdocs.yml` in place** (`MkdocsYmlPatcher.SetLogo`). We line-edit `theme.logo` rather than parsing and re-emitting the YAML so users' hand-written comments/formatting survive. Option 1 of the three sketched in #98 — in-place edit of the repo-root `mkdocs.yml` — fits the current one-wiki-per-checkout usage; multi-wiki-per-checkout can move to a per-wiki override file when it actually comes up.
- **Endpoints are gated by the same token + Origin/port check as the other write-back endpoints.** Kestrel body limit bumped 1 MB → 3 MB to fit multipart logo uploads (client-side hard-cap is 2 MB, extension allowlist is png/jpg/jpeg/svg/webp/gif/ico). Filenames are stripped of anything but `[A-Za-z0-9-_]` before landing on disk; the path safety check refuses anything that escapes `wiki/assets/`.
- **Livereload guard** (`EAxEditGuard`, from #96) is held while any brand-editor field is focused, so an unrelated `status/*.md` refresh doesn't wipe in-progress picker edits.
- **Orphan cleanup whitelists `assets/`.** `CleanupOrphanedFilesAsync` deletes unknown top-level directories in the output dir on every run; `assets/` is in the recognized special-dirs set so uploaded logos survive re-exports.
- **Brand palette** (from issue #79): Light Cyan `#C4E5E7`, Jet Black `#103135`, Platinum `#F3F7F7`, Opal `#A8C6C7`, Lime Cream `#D0F391`. Fonts Geist + Geist Mono are pulled from Google Fonts via an `@import` at the top of `brand.css` only. The header + navigation tab band uses Light Cyan with Jet Black text/icons (via `color` overrides on `.md-header`/`.md-tabs`, which `currentcolor`/`inherit` propagate to buttons and tab links); `:root` theme variables stay dark so the search field, footer and other surfaces are unaffected.
- **Graph layer colors used by the EurSuRA sample wiki** — copy these into `wiki/brand.css` if you want the same palette on the graph nodes:
...
- **Production target is local server only** — the write-back server requires a running Windows machine with EA installed. The wiki is served locally via MkDocs. GitHub Pages is supported as a read-only publishing target for the same generated `wiki/` folder.
- **GitHub Pages** at `https://hvroosmalen-eaxpertise.github.io/EAxWiki/` — used for the test/demo model.
- **The Relationship Graph works on GitHub Pages too** (issue #84). The graph's `fetch('graph-index.json')` used to resolve wiki-root-relative URLs by counting slashes in `window.location.pathname`, which assumed the site was served at the domain root. On GH Pages project sites (`/EAxWiki/…`), a nested page hit `.../graph-index.json` at the host root instead — 404 every time — and every page fell through to *Graph data unavailable*. Fix: derive the wiki base from the `<script src="cytoscape.min.js">` tag mkdocs already emits with the correct relative path per page. `resolveUrl` and `wikiBase` both go through this helper now, so any base-path prefix works.
...
## Server hardening (August 2026 audit — issues #83, #85, #86)

_2026-08-12_

- **Edit pencils gate on a runtime `/readyz` probe** (issue #83): a small `api-probe.js` runs once per page load and toggles `body.ea-api-ready` vs `body.ea-api-unavailable` based on the probe result. `extra.css` starts the three edit-button classes at `opacity: 0.35; cursor: not-allowed` and only enables them under `body.ea-api-ready` — no enabled→disabled flash. Tooltips distinguish "API not reachable" vs "API up but no EA model" so a viewer on a machine without EA sees why editing is off, not just a broken pencil. The three editors also bail out of `enterEditMode` when the class is absent, so a keyboard-focused click on a greyed pencil does nothing.
- **`/readyz` stays truthful across the server's lifetime** (issue #85): originally a `bool eaReady` set once at startup — so a dropped COM connection was invisible to the endpoint. Two signals now combine: `EaReaderStaDispatcher.IsHealthy` (volatile bool the STA pump flips false the moment a work item hits a `COMException`, back to true on the next success — zero probe traffic, reflects real user activity) and a cached fallback probe on `/readyz` that runs `GetStatusTypes()` at most once per 15s to catch the "server idle for hours, EA quietly died" case. `IEaReader.IsHealthy` has a default `true` implementation so tests and callers that don't track this signal need no changes.
- **Editor JS blobs extracted from `InfrastructureWriter` into embedded `Resources/*.js`** (issue #85). `InfrastructureWriter` was 1263 lines because every browser-side script — `status-editor.js`, `notes-editor.js`, `row-notes-editor.js`, `api-probe.js`, `ea-icons.js`, `graph-init.js` — lived as a C# raw-string literal: ~800 lines of untyped, unlinted, un-diffable JavaScript. Now each ships as a real `.js` file loaded through a small `EmbeddedResource.ReadText` helper (per-process cache), same pattern the `extra.css` / `cytoscape.min.js` resources already used. `graph-init.js.tmpl` keeps the per-brand color substitution via `/*EA_LAYER_COLORS*/` and `/*EA_LAYER_DARK_TEXT*/` placeholder markers. `InfrastructureWriter` drops to 211 lines and every JS change is now a proper diff on a proper file.
- **Write-back endpoints are async end-to-end** (issue #85). Every EA-touching endpoint used to be a synchronous lambda that ended up in `EaReaderStaDispatcher.Dispatch<T>` blocking on `tcs.Task.GetAwaiter().GetResult()` — a single COM `Update()` can run for 500ms+ and consumed a thread-pool worker for the whole hop. Now the dispatcher exposes `DispatchAsync`/`DispatchVoidAsync` that return the underlying `Task` unblocked, `IEaReader` gains async siblings for `GetStatusTypes` / `GetElementSummary` / `GetDiagramSummary` / the seven `Update…` methods (default implementations wrap the sync calls; the STA dispatcher overrides them with real async), and every `/api/*` endpoint that hits EA (`/api/status`, `/api/notes`, `/api/diagram-notes`, `/api/row-notes`, `/api/status-types`, and the two `/api/ai-suggest*` endpoints) `await`s the async version and passes `HttpContext.RequestAborted`. The STA thread stays single-threaded (COM requires that) — only the ASP.NET side is freed, so a concurrent 3-minute AI-suggest no longer starves other editors.
- **`WikiWritebackServer.Configure` is now separately callable** (issue #86 harness). `RunAsync` split into a production bootstrap (Kestrel + STA dispatcher + ready-file lifetime hooks) and an internal `Configure(WebApplication, IEaReader, Config, string, ILogger)` method that installs all middleware and endpoints on an already-built `WebApplication`. Enables an 11-test HTTP-level suite under `Microsoft.AspNetCore.TestHost` against a `FakeEaReader` — no ports, no Kestrel, no COM. Covers auth (missing/wrong token → 401 with a **regression guard that the response body never contains either the expected or provided token**), happy write path, validation (unknown status → 400, missing file → 404, path traversal → 400), readyz truthfulness on both branches, and CORS origin-port matching. `EAxWiki` main assembly gained `InternalsVisibleTo(EAxWiki.Tests)` so tests can invoke `Configure`.
- **Small audit fixes (issues #83/#85 follow-through):**
    - **Token no longer leaks into logs or 401 responses.** `ApiTokenStore` logs a `first4…last4` fingerprint instead of the full token; the 401 body no longer echoes either `expected` or `provided`. Logging the real token defeated the point of the `FixedTimeEquals` compare on the request path.
    - **`FrontmatterParser.UpdateStatus` preserves the file's original line endings**, matching `UpdateNotes` / `UpdateRowNotes` / `UpdatePackageNotes`. Previously used `File.WriteAllLines` which normalizes to `Environment.NewLine`, so a status write-back on Linux silently rewrote CRLF → LF for the whole page. Also promoted its two regexes to compiled statics.
    - **`ModelMapper.MapElement` guards the `eaElement.Modified` COM cast** with the same `as DateTime?` pattern already used for `Created`. Orphaned elements returning null no longer crash the whole export.
    - **`--api-port` / `--wiki-port` use `int.TryParse` with a 1-65535 range check** and emit a clear error message on bad input.
    - **AI endpoints share a single static `HttpClient`** instead of `using var httpClient = new HttpClient` per request. Avoids socket exhaustion under concurrent LLM calls.
- **HtmlSanitizer 9.0.892 → 9.1.973** — picked up transitively during the async refactor, resolves the `NU1902` warning about the AngleSharp vulnerability in the older transitive dep.

...
- **`EaReader`** reads the EA model via COM, exposing `MapDiagram`, `ExportDiagramImage`, `GetStatusTypes`, `UpdateElementStatus`, and `RepositoryPath` methods. Validates the repository path before opening and uses `is` pattern matching over all COM collection loops to avoid hard casts.
...
- **Wiki write-back server** (`WikiWritebackServer`, `--api` mode): an ASP.NET Core minimal API (SDK: `Microsoft.NET.Sdk.Web`) running on a configurable port (default 8001). Handles `POST /api/status` requests from the widget. Validates the new status against live `t_statustypes`, updates EA via `element.Update()` COM call, then patches the `.md` file in-place.

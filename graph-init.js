...
function _write(key, positions) {
    try { localStorage.setItem(key, JSON.stringify(positions)); return true; }
...
function restoreLayout(cy) {
    var positions = _read(_bookmarkKey);
    if (!positions) { appendStatus('No bookmarked layout — press Save first.'); return false; }
    var layout = cy.layout();
    if (layout && layout.stop) { try { layout.stop(); } catch (e) {} }
    var applied = _applyPositions(cy, positions);
    cy.fit(cy.elements(), 40);
    // Restore also resets the working state to the bookmark so a livereload right after
    // Restore doesn't snap back to the pre-restore drag positions.
    _write(_workingKey, positions);
    appendStatus(applied > 0 ? ('Restored bookmark (' + applied + ' nodes).') : 'No matching nodes to restore.');
    return applied > 0;
}
...
function initEaGraph() {
    ...
    toolbar.style.cssText = 'margin-bottom:8px;display:flex;gap:6px;';
    var btnStyle = 'display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;padding:0;cursor:pointer;background:var(--md-default-bg-color,#fff);border:1px solid var(--md-default-fg-color--lightest,#ccc);border-radius:4px;color:var(--md-default-fg-color,#333);';
    ...
    var saveBtn = makeBtn('ea-save-layout');
    var restoreBtn = makeBtn('ea-restore-layout');
    var clearBtn = makeBtn('ea-clear-layout');
    toolbar.appendChild(saveBtn);
    toolbar.appendChild(restoreBtn);
    toolbar.appendChild(clearBtn);
    container.parentNode.insertBefore(toolbar, container);

    if (typeof EAxIcons !== 'undefined') {
        EAxIcons.set(saveBtn,    'save-layout',    'Save layout and download image');
        EAxIcons.set(restoreBtn, 'restore-layout', 'Restore saved layout');
        EAxIcons.set(clearBtn,   'clear-layout',   'Clear saved layout');
        [saveBtn, restoreBtn, clearBtn].forEach(function (b) {
            var svg = b.querySelector('svg');
            if (svg) svg.setAttribute('style', iconStyle);
        });
    } else {
        // Fallback if ea-icons.js didn't load — keep the widget usable.

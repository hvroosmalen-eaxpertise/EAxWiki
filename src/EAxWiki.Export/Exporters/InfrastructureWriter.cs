...
/* Row-level notes editor (attribute / method / tagged-value descriptions) */
.ea-row-notes-text {
  color: var(--md-default-fg-color);
}
.ea-row-notes-placeholder {
  color: var(--md-default-fg-color--light);
}
.ea-row-notes-edit-btn {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  width: 1.6em;

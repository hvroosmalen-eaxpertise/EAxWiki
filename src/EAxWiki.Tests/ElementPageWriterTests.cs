...
public class ElementPageWriterTests
{
    ...
    #region RelationshipGraphRenderer

    [Fact]
    public void RelationshipGraphRenderer_NoConnections_ReturnsEmpty()
    {
        var element = MakeElement();
        var ctx = PlainContext();
    ...
    [Fact]
    public void RelationshipGraphRenderer_OneHop_ReturnsGraphDiv()
    {
        var neighbor = new EaElement { Id = 2, Name = "Neighbor", PackageId = 10 };
        var element = MakeElement(e => e.Connectors = [new EaConnector { Id = 1, Name = "link", Type = "Association", SourceId = 1, TargetId = 2 }]);
        var ctx = new ExportContext(@"C:\out",
            [(neighbor, @"C:\pkg")],
            new Dictionary<int, (EaElement, string)> { [2] = (neighbor, @"C:\pkg"), [1] = (element, @"C:\pkg") },
            [], [], [],
            new Dictionary<int, (string, int?)> { [10] = ("Pkg", null) }) { ApiPort = 0 };
        var result = RelationshipGraphRenderer.Render(element, @"C:\pkg", ctx);
        Assert.Contains("ea-graph-container", result);
        Assert.Contains("ea-graph-data", result);
    ...
    [Fact]
    public void RelationshipGraphRenderer_ConnectorToSelf_Excluded()
    {
        var element = MakeElement(e => e.Connectors = [new EaConnector { Id = 1, Name = "self", Type = "Self", SourceId = 1, TargetId = 1 }]);
        var ctx = new ExportContext(@"C:\out", [], new Dictionary<int, (EaElement, string)> { [1] = (element, @"C:\pkg") },
            [], [], [],
            new Dictionary<int, (string, int?)>()) { ApiPort = 0 };
        var result = RelationshipGraphRenderer.Render(element, @"C:\pkg", ctx);
        Assert.Empty(result);
